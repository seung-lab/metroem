...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Upsample H5 Field")
    parser.add_argument(
        "--src_path", type=str, help="Path to H5 file, with dataset stored in main"
    )
    parser.add_argument(
        "--dst_path", type=str, help="Path to H5 file which will be upsampled"
    )
    ...
    upsample_args = [
        (args.src_path, args.src_mip, args.dst_mip, n_start, n_stop, device)
    ...
    def upsample_field(upargs):
        ...
        src_dset = src_h5["main"]
        dst_path = src_path.parent / "tmp_{}_{}_MIP{}.h5".format(
            n_start, n_stop, dst_mip
        )
        ...
        hsz = (src_dset.shape[-1] - dst_dset.shape[-1]) // 2
        with torch.no_grad():
            for n in range(n_start, n_stop):
                ...
                dst_field_cropped = dst_field[0, :, hsz:-hsz, hsz:-hsz]
